...
function createCipher(session, key, decipher, disableIV = false) {
	...
	const cipher = forge.cipher[decipher ? "createDecipher" : "createCipher"](
		"AES-CBC",
		md5(key)
	);
	const iv = disableIV
		? new forge.util.createBuffer(
...
function deflate(data) {
	return pako.deflateRaw(new forge.util.ByteBuffer(data).toHex(), {
		level: 6,
		to: "string",
	});
}
...
function getLoginKey(username, password, scramble, fromCas) {
	const hash = forge.md.sha256
		...
		.digest();
	const key =
		(fromCas ? "" : username.toLowerCase()) + hash.toHex().toUpperCase();
